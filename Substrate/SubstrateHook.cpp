...
static size_t SubstrateHookFunctionARM(SubstrateProcessRef process, void *symbol, void *replace, void **result) {
     ...
     MSLog(MSLogLevelNotice, "ARM call SubstrateHookFunctionARM ",0);
    ...
    if (MSDebug) {
        char name[16];
        sprintf(name, "%p", area);
        MSLog(MSLogLevelNotice, "symbol hex: ",0);
    ...
    *result = buffer;
    ...
    if (MSDebug) {
        char name[16];
        sprintf(name, "%p", *result);
        MSLog(MSLogLevelNotice, "*result hex: ",0);
    ...
    if (MSDebug) {
        char name[16];
        sprintf(name, "%p", area);
        MSLog(MSLogLevelNotice, "newFun hex: ",0);
        MSLogHexEx(area, used + sizeof(uint32_t), 4, name);
    }
...
static size_t SubstrateHookFunction(SubstrateProcessRef process, void *symbol, void *replace, void **result) {
    if (MSDebug)
        MSLog(MSLogLevelNotice, "ARM call",0);
    ...
    if ((reinterpret_cast<uintptr_t>(symbol) & 0x1) == 0)
    {
        ...
        MSLog(MSLogLevelNotice, "ARM call SubstrateHookFunctionThumb",0);
...
static void SubstrateHookFunction(SubstrateProcessRef process, void *symbol, void *replace, void **result) {
    ...
    if (MSDebug)
        MSLog(MSLogLevelNotice, "__x86_64__ call",0);
...
_extern void MSHookFunction(void *symbol, void *replace, void **result) {
    ...
    MSLog(MSLogLevelNotice, "call MSHookFunction %p %p %p",symbol, replace, result);

    MSLogHexEx(symbol,16,16,0);

    SubstrateHookFunction(NULL, symbol, replace, result);

    MSLog(MSLogLevelNotice, "call SubstrateHookFunction  %p", result);
    MSLogHexEx(result,4,4,0);
}
